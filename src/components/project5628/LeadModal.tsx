...
export function LeadModal({ open, onClose, prefill }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("https://functions.poehali.dev/224b976f-276f-46b9-a317-78cbf9185917", {
        method: "POST",
      ...
      // Если бэкенд недоступен — всё равно показываем «Спасибо»
      setDone(true)
    } finally {
      setLoading(false)
    }
      ...
      >
        ...
        {done ? (
          <div className="p-10 text-center">
            ...
            <button
              onClick={handleClose}
              className="mt-8 bg-foreground text-white px-8 py-3 text-sm font-medium hover:bg-foreground/90 transition-colors"
            >
              Закрыть
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8">
            <p className="text-xs text-muted-foreground tracking-[0.2em] uppercase mb-2">Проект 5628</p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ваши пожелания, вопросы (необязательно)"
                  rows={3}
                  className="w-full border border-border px-4 py-3 text-sm outline-none focus:border-foreground transition-colors resize-none bg-background"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-6 w-full bg-foreground text-white py-4 text-sm font-medium hover:bg-foreground/90 transition-colors disabled:opacity-60"
            >
