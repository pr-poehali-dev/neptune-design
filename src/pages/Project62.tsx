...
import { calculatePrice, formatRub, defaultState, CalcState } from "@/components/project62/calcLogic"

const PHOTOS = [
  { url: "https://cdn.poehali.dev/projects/45e5dd28-acf9-4577-bd02-33b9654b7bee/bucket/7e0b2b28-45c2-4057-822b-238b64242a19.jpg", label: "Вид с фасада" },
  { url: "https://cdn.poehali.dev/projects/45e5dd28-acf9-4577-bd02-33b9654b7bee/bucket/59e985ab-f2e4-499d-a5ab-e360ced49961.jpg", label: "Вид сбоку" },
]
const AREAS = [62, 80, 100, 120, 150, 200]

const SPECS = [
  { label: "Общая площадь", value: "62,5 м²" },
  { label: "Жилая площадь", value: "34,8 м²" },
  { label: "Размеры дома", value: "7 × 7 м" },
  { label: "Высота здания", value: "~8 м" },
  ...
  { label: "Стены", value: "Газобетон / Пеноблок" },
  { label: "Кровля", value: "Битумная черепица" },
]

const ADVANTAGES = [
  { num: "01", title: "Компактный — экономичный", text: "Небольшая площадь = меньше расходов на строительство, отопление и обслуживание." },
  { num: "02", title: "Мансарда — больше места", text: "Две спальни на мансарде — полноценные жилые комнаты с хорошей высотой потолков." },
  { num: "03", title: "Простая и понятная планировка", text: "Всё на своём месте: гостиная, кухня и санузел внизу, обе спальни — наверху." },
  { num: "04", title: "Быстро строится", text: "Небольшой дом возводится значительно быстрее, чем большой: от фундамента до сдачи — меньше времени." },
  { num: "05", title: "Подходит для небольшой семьи", text: "Оптимальный выбор для пары или семьи с одним ребёнком — всего достаточно, ничего лишнего." },
  { num: "06", title: "Надёжный фундамент", text: "Монолитная плита — лучший вариант для небольших домов: равномерная нагрузка, долгий срок службы." },
]
...
function exportEstimate(s: CalcState, total: number, perM2: number) {
  ...
  const html = `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>Расчёт — Проект 62</title><style>body{font-family:Arial,sans-serif;max-width:700px;margin:40px auto;color:#1a1a1a}.price{font-size:36px;font-weight:700;margin:20px 0 4px}.perm2{color:#666;font-size:15px;margin-bottom:28px}table{width:100%;border-collapse:collapse;font-size:14px}.disc{margin-top:28px;padding:14px;background:#f5f5f5;font-size:13px;color:#666;border-left:3px solid #ccc}</style></head><body><h1>Проект 62 — Расчёт стоимости</h1><p style="color:#888;font-size:13px">Сформировано: ${dateStr}</p><div class="price">${formatRub(total)}</div><div class="perm2">${formatRub(perM2)} за м²</div><table>${rows}</table><div class="disc">Расчёт предварительный. Итог — после изучения участка и согласования ТЗ.</div></body></html>`
  const blob = new Blob([html], { type: "text/html;charset=utf-8" })
  const url = URL.createObjectURL(blob)
  const a = document.createElement("a")
  a.href = url; a.download = `raschet-project-62-${now.toISOString().slice(0, 10)}.html`; a.click()
  URL.revokeObjectURL(url)
}

export default function Project62() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[92vh] flex items-end">
        <div className="absolute inset-0 z-0">
          <img src={PHOTOS[0].url} alt="Проект 62" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-black/10" />
        </div>
        <div className="absolute top-8 left-8 z-10">
          <span className="bg-white/10 backdrop-blur-sm border border-white/20 text-white text-xs tracking-[0.2em] uppercase px-4 py-2">Проект № 62</span>
        </div>
        <div className="relative z-10 w-full px-6 md:px-12 lg:px-20 pb-16 md:pb-24">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-medium text-white leading-tight mb-6">Компактный дом<br />с мансардой</h1>
            <p className="text-white/75 text-lg mb-10">62,5 м² · 7 × 7 м · 2 спальни · 3 комнаты</p>
        ...
        <div className="container mx-auto px-6 md:px-12">
          <div className="grid md:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <p className="text-muted-foreground leading-relaxed">Небольшой дом из газобетона — проверенное решение: лёгкий фундамент, тёплые стены и быстрый монтаж без потери в качестве.</p>
            </div>
            <div className="space-y-6">
              {[
                { title: "Фундамент", detail: "Монолитная плита", text: "Оптимален для компактного дома — равномерно распределяет нагрузку и защищает от грунтовой влаги." },
                { title: "Стены", detail: "Газобетон / Пеноблок", text: "Лёгкий и тёплый материал с хорошей теплоизоляцией. Снижает расходы на отопление." },
                { title: "Перекрытия", detail: "Деревянные балки", text: "Классическое решение для лёгких мансардных конструкций: надёжно, тепло, экономично." },
                { title: "Кровля", detail: "Битумная черепица, многоскатная", text: "Мягкая черепица хорошо держит форму на сложных кровлях, морозостойкая и тихая под дождь." },
              ].map((item) => (
      ...
      <section className="py-24 bg-secondary/30">
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="p-4 bg-secondary/40 text-sm text-muted-foreground text-center">
            1 этаж: Тамбур ~3,1 м² · Санузел ~5 м² · Бойлерная ~3,1 м² · Гостиная ~12 м² · Кухня ~8,8 м²<br />
            Мансарда: Холл ~2,7 м² · Санузел ~5 м² · Спальня ~11,2 м² · Спальня ~11,4 м²
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-background">
        <div className="container mx-auto px-6 md:px-12">
          <div className="grid md:grid-cols-2 gap-x-16 gap-y-4 items-start mb-16">
            ...
            <p className="text-muted-foreground leading-relaxed self-end">Маленький дом — это не компромисс, а грамотный выбор. Комфорт, экономия и быстрое строительство.</p>
          </div>
        ...
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white border border-border p-8">
              ...
              <p className="text-3xl font-medium mb-2">от 35 100 ₽</p>
            </div>
            <div className="bg-foreground text-white p-8">
              <p className="text-white/60 text-sm mb-2">Полный проект (с инженерией)</p>
              <p className="text-3xl font-medium mb-2">от 84 100 ₽</p>
            </div>
          </div>
          <p className="text-muted-foreground text-sm">Дополнительно можно заказать: смету, инженерию, дизайн интерьера, изменения проекта.</p>
        </div>
      </section>
          ...
          <div className="container mx-auto px-6 md:px-12">
            ...
            <div className="grid lg:grid-cols-[1fr_360px] gap-10 items-start">
              <div>
                ...
                <OptionGroup label="Кровельное покрытие" options={["Битумная черепица", "Металлочерепица", "Мягкая черепица"]} value={s.roof} onChange={set("roof")} />
      ...
      <ContactModal open={leadOpen} onClose={() => setLeadOpen(false)} projectName="Проект 62" prefill={leadData} />
      <FloatingCTA />
    </div>
  )
}