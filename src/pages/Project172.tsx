...
const PHOTOS = [
  { url: "https://cdn.poehali.dev/projects/45e5dd28-acf9-4577-bd02-33b9654b7bee/bucket/c357a737-19ea-4ff9-bf35-eb06987483d9.jpg", label: "Вид с фасада" },
  { url: "https://cdn.poehali.dev/projects/45e5dd28-acf9-4577-bd02-33b9654b7bee/bucket/07a61063-0e2f-488a-960c-e9eba49f2672.jpg", label: "Планировка" },
]
const AREAS = [100, 130, 150, 172, 200, 250, 300, 350, 400]

const SPECS = [
  { label: "Общая площадь", value: "172,6 м²" },
  { label: "Жилая площадь", value: "82,9 м²" },
  { label: "Размеры дома", value: "13,4 × 12,9 м" },
  { label: "Высота здания", value: "9,6 м" },
  { label: "Комнат", value: "4" },
  { label: "Спален", value: "3" },
  { label: "Гараж", value: "Встроенный" },
  { label: "Мансарда", value: "Есть" },
...
const ADVANTAGES = [
  { num: "01", title: "Гараж внутри дома", text: "Встроенный гараж на 1 этаже — заезжаете прямо из дома, не выходя на улицу." },
  { num: "02", title: "Мансарда — полноценный этаж", text: "Две просторные спальни и санузел на мансарде: отдельная зона для взрослых или детей." },
  { num: "03", title: "Подвал для технических нужд", text: "Всё инженерное оборудование в подвале — жилые этажи свободны от котла и систем." },
  { num: "04", title: "Терраса для отдыха", text: "Просторная терраса выходит в сад — идеальное место для семейных ужинов на свежем воздухе." },
  { num: "05", title: "Надёжный конструктив", text: "Газобетонные блоки с облицовкой кирпичом: тепло, долговечно, без ежегодного ухода." },
  { num: "06", title: "Продуманное зонирование", text: "Гостиная и кухня на 1 этаже, спальни — на мансарде. Общее и личное пространство не пересекаются." },
]
...
function exportEstimate(s: CalcState, total: number, perM2: number) {
  ...
  const html = `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>Расчёт — Проект 172</title><style>body{font-family:Arial,sans-serif;max-width:700px;margin:40px auto;color:#1a1a1a}.price{font-size:36px;font-weight:700;margin:20px 0 4px}.perm2{color:#666;font-size:15px;margin-bottom:28px}table{width:100%;border-collapse:collapse;font-size:14px}.disc{margin-top:28px;padding:14px;background:#f5f5f5;font-size:13px;color:#666;border-left:3px solid #ccc}</style></head><body><h1>Проект 172 — Расчёт стоимости</h1><p style="color:#888;font-size:13px">Сформировано: ${dateStr}</p><div class="price">${formatRub(total)}</div><div class="perm2">${formatRub(perM2)} за м²</div><table>${rows}</table><div class="disc">Расчёт предварительный. Итог — после изучения участка и согласования ТЗ.</div></body></html>`
  const blob = new Blob([html], { type: "text/html;charset=utf-8" })
  const url = URL.createObjectURL(blob)
  const a = document.createElement("a")
  a.href = url; a.download = `raschet-project-172-${now.toISOString().slice(0, 10)}.html`; a.click()
  URL.revokeObjectURL(url)
}

export default function Project172() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[92vh] flex items-end">
        <div className="absolute inset-0 z-0">
          <img src={PHOTOS[0].url} alt="Проект 172" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-black/10" />
        </div>
        <div className="absolute top-8 left-8 z-10">
          <span className="bg-white/10 backdrop-blur-sm border border-white/20 text-white text-xs tracking-[0.2em] uppercase px-4 py-2">Проект № 172</span>
        </div>
        <div className="relative z-10 w-full px-6 md:px-12 lg:px-20 pb-16 md:pb-24">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-medium text-white leading-tight mb-6">Дом с мансардой<br />и встроенным гаражом</h1>
            <p className="text-white/75 text-lg mb-10">172,6 м² · 13,4 × 12,9 м · 3 спальни · 4 комнаты</p>
        ...
        <div className="container mx-auto px-6 md:px-12">
          <div className="grid md:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <p className="text-muted-foreground leading-relaxed">Проект построен на сочетании газобетонных блоков с облицовкой кирпичом: тёплые стены с долговечным и красивым фасадом без ежегодного обслуживания.</p>
            </div>
            <div className="space-y-6">
              {[
                { title: "Фундамент", detail: "Ленточный", text: "Надёжное основание для дома с подвальным уровнем и нагрузкой двух этажей." },
                { title: "Стены", detail: "Газобетон + облицовочный кирпич", text: "Газобетон обеспечивает тепло, кирпичная облицовка — прочность и эстетику на десятилетия." },
                { title: "Перекрытия", detail: "Железобетонные плиты", text: "Заводские плиты: высокая несущая способность, быстрый монтаж и отличная звукоизоляция." },
                { title: "Кровля", detail: "Металлочерепица, многоскатная", text: "Долговечное покрытие с минимальным уходом, устойчивое к российскому климату." },
              ].map((item) => (
      ...
      <section className="py-24 bg-secondary/30">
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-2 gap-3">
            {PHOTOS.map((photo, i) => (
              <button key={i} onClick={() => setLightbox(i)} className="group relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all duration-300 flex items-center justify-center">
                  <span className="opacity-0 group-hover:opacity-100 text-white text-xs tracking-widest uppercase">Открыть</span>
                </div>
              </button>
            ))}
          </div>
          <div className="mt-8 p-4 bg-secondary/40 text-sm text-muted-foreground text-center">
            Планировка 1 этажа: Тамбур · Холл · Гостиная · Кухня · Санузел · Гараж · Бойлерная · Терраса<br />
            Мансарда: Холл · 2 спальни · Санузел · Подвал: Технические помещения
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-background">
        <div className="container mx-auto px-6 md:px-12">
          <div className="grid md:grid-cols-2 gap-x-16 gap-y-4 items-start mb-16">
            ...
            <p className="text-muted-foreground leading-relaxed self-end">Удобная планировка, надёжные материалы и продуманные зоны — всё для комфортной жизни большой семьи.</p>
          </div>
      ...
      <section className="py-16 bg-secondary/30">
        <div className="container mx-auto px-6 md:px-12">
          <p className="text-muted-foreground text-xs tracking-[0.3em] uppercase mb-6">Стоимость документации</p>
          <h2 className="text-3xl font-medium mb-10">Цена проекта</h2>
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white border border-border p-8">
              <p className="text-muted-foreground text-sm mb-2">Архитектурный + конструктивный раздел</p>
              <p className="text-3xl font-medium mb-2">от 46 800 ₽</p>
            </div>
            <div className="bg-foreground text-white p-8">
              <p className="text-white/60 text-sm mb-2">Полный проект (с инженерией)</p>
              <p className="text-3xl font-medium mb-2">от 131 570 ₽</p>
            </div>
          </div>
          <p className="text-muted-foreground text-sm">Дополнительно можно заказать: смету, дизайн интерьера, инженерные решения, изменения проекта.</p>
        </div>
      </section>
          ...
          <div className="container mx-auto px-6 md:px-12">
            ...
            <div className="grid lg:grid-cols-[1fr_360px] gap-10 items-start">
              <div>
                ...
                <OptionGroup label="Основание дома" options={["Монолитная плита", "УШП плита", "Финский фундамент"]} value={s.foundation} onChange={set("foundation")} />
                ...
                <OptionGroup label="Материал стен" options={["Газобетон + облицовочный кирпич", "Газобетон", "Кирпич"]} value={s.wall} onChange={set("wall")} />
                <OptionGroup label="Перекрытия" options={["Железобетонные плиты", "Монолитные плиты перекрытия", "Деревянные балки"]} value={s.slab} onChange={set("slab")} />
                <OptionGroup label="Кровельное покрытие" options={["Металлочерепица", "Мягкая черепица", "Керамическая черепица"]} value={s.roof} onChange={set("roof")} />
                ...
                <OptionGroup label="Отделка фасада" options={["Облицовочный кирпич", "Штукатурка", "Сайдинг"]} value={s.facade} onChange={set("facade")} />
